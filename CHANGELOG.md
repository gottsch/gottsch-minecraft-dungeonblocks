...
- **Dungeon Banner** — a two-block-tall wall banner with a single fixed design, narrow like a vanilla banner but tapering to a point instead of ending square. The cloth waves gently and continuously, rippling from the rod down to the hem; neighbouring banners are offset from each other so a row of them does not move as one sheet. Hangs from its rod, so only the top block needs a wall behind it — a banner can drape over a doorway — and breaking either half takes the whole thing down for one item. It is **not** a vanilla banner: no patterns, no dyes and no loom — one banner, one design, walk-through like cloth should be.
- **Eight banners, in three factions.** The crimson set runs **Dungeon Banner**, **Grimy Banner** and **Tattered Banner**. The **Orc War Banner** set is daubed on filthy sackcloth with a slit-pupil eye and claw marks, and comes plain, **Tattered** and **Bloodstained**. The **Undead Banner** is grave linen, near black, carrying a bone skull over a spine, and comes plain and **Tattered**. The wear is not cosmetic labelling: tattered banners have genuinely ragged hems and holes torn clean through the cloth that you can see the wall through — though never through the crest itself, so a banner always stays identifiable. All eight share one model, one block entity type and one renderer; the faction and the condition live entirely in the texture. The artwork is placeholder for now.
- **A banner's motion can be switched off per banner** — right-click one with an empty hand and its cloth goes still; right-click again and it resumes. The state is a real `animated` blockstate, so it survives saving and can be set by a structure or datapack. `animateBanners` in the new **visuals** section of the client config is a master switch over the top of that: turning it off stills every banner on your own client, but it will never animate a banner somebody deliberately stilled. Still banners keep a slight bow so they read as cloth rather than as a painted board, and both switches take effect immediately — no reload or chunk rebuild.
